...
func resourceDataset(d *schema.ResourceData, meta interface{}) (*bigquery.Dataset, error) {
	...
	dataset := &bigquery.Dataset{DatasetReference: &bigquery.DatasetReference{
...
func resourceBigQueryDatasetUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = config.clientBigQuery.Datasets.Update(projectID, datasetID, dataset).Do()
	if err != nil {
		return err
	}
...
func resourceBigQueryDatasetDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("retain_on_delete").(bool) {
		log.Printf("[INFO] Removing dataset %q with `retain_on_delete` set. Please delete this dataset manually.", d.Id())
		d.SetId("")
		return nil
	}
	...
	err := config.clientBigQuery.Datasets.Delete(projectID, datasetID).Do()
	if err != nil {
		return err
	}
