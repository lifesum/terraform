...
func resourceTable(d *schema.ResourceData, meta interface{}) (*bigquery.Table, error) {
	...
	if v, ok := d.GetOk("schema"); ok {
		schemaJSON, err := normalizeJsonString(v.(string))
		if err != nil {
			return nil, err
		}

		schema, err := expandSchema(schemaJSON)
		if err != nil {
			return nil, err
		}
...
func resourceBigQueryTableRead(d *schema.ResourceData, meta interface{}) error {
	...
	if err := d.Set("schema", flattenSchema(res.Schema)); err != nil {
		return err
	}
...
func resourceBigQueryTableUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	_, err = config.clientBigQuery.Tables.Update(projectID, datasetID, tableID, table).Do()
	if err != nil {
		return err
	}
...
func resourceBigQueryTableDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if d.Get("retain_on_delete").(bool) {
		log.Printf("[WARN] Removing table %q with `retain_on_delete` set. Please delete this table manually.", d.Id())
		d.SetId("")
		return nil
	}
	...
	err := config.clientBigQuery.Tables.Delete(projectID, datasetID, tableID).Do()
	if err != nil {
		return err
	}
...
func flattenSchema(tableSchema *bigquery.TableSchema) string {
	...
	jsonSchema, err = normalizeJsonString(string(schema))
	if err != nil {
		return jsonSchema
	}
